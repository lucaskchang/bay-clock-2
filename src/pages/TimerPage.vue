  ...
  <div class="window-height row items-center text-center full-height full-width bg-blue">
    <div class="col items-center">
      <div v-if="tab == 'pomodoro'" class="column justify-center items-center">
        <q-input
          v-if="pomodoro_state == 'work' || pomodoro_state == 'none'"
          v-model="course"
          placeholder="I'm working on..."
          label-color="white"
          color="white"
          input-class="text-h3 text-white text-center"
          hide-bottom-space
          borderless
        />
    ...
    <q-dialog v-model="creditsModal">
      <q-card style="width: 500px; max-width: 80vw">
        <q-card-section class="text-h5"> Credits </q-card-section>
        <q-card-section>
          <p>
            The original idea for homework timers on Bay Clock was part of Ella Yacubian's
...
function start_pomodoro() {
  ...
  setInterval(() => {
    time_left.value -= 1;
    pomodoro_timer.value =
      Math.floor(time_left.value / 60).toString() +
      ":" +
      (Math.floor(time_left.value % 60) < 10
        ? "0" + Math.floor(time_left.value % 60)
        : Math.floor(time_left.value % 60)
      ).toString();
