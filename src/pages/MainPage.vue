...
<div class="q-ma-md">
  ...
  >
      <q-linear-progress
        ...
        size="25px" :color="colorPalette[barColor]"
  ...
  <div class="row justify-between q-ma-lg">
    <div class="row">
      <q-btn class="q-ma-sm" :color="colorPalette[buttonColors['Links']]" label="Useful Links" @click="linksMenu = true" rounded no-caps />
      <q-btn class="q-ma-sm" :color="colorPalette[buttonColors['Menu']]" label="Lunch Menu" @click="lunchMenu = true" rounded no-caps />
      <q-btn class="q-ma-sm" :color="colorPalette[buttonColors['Schedule']]" label="Custom Schedule" @click="scheduleMenu = true" rounded no-caps />
      <q-btn class="q-ma-sm" :color="colorPalette[buttonColors['Styles']]" label="Customize" @click="styleMenu = true" rounded no-caps />
    </div>
    <div class="row">
      <q-btn class="q-ma-sm" :color="colorPalette[buttonColors['Weekly']]" label="Weekly Schedule" @click="weekMenu = true" rounded no-caps />
      <q-toggle v-if="holiday != ''" class="q-ma-sm" v-model="holidayBool" :label="holiday" />
    </div>
  </div>
      ...
      <q-card-section>
        ...
        >
          ...
          <div>
            ...
            <q-btn label="Cancel" @click="scheduleMenu = false" color="negative" flat class="float-right" />
          </div>
        </q-form>
      </q-card-section>
    </q-card>
  ...
  >
    <q-card style="width: 900px; max-width: 80vw;">
            ...
            >
              <q-tab-panel name="colors" class="q-pt-none">
                ...
                <div class="q-gutter-sm">
                  <q-btn
                    v-for="(color, hex) of Object.fromEntries(Object.entries(colorPalette).slice(0,5))"
                    :key="color"
                    :color="color"
      ...
      <div class="q-pa-lg">
        ...
        <q-btn label="Cancel" @click="styleMenu = false" color="negative" flat class="float-right" />
      </div>
    </q-card>
  </q-dialog>
  ...
  <div class="float-bottom text-subtitle1 text-center q-pt-xl q-mt-xl">
    <p class="q-ma-none q-mt-xl q-mp-xl">Created by <a href="https://lucaskchang.com" target="_blank">Lucas Chang</a></p>
    <p class="q-mt-sm"><a class="footer-link" href="https://github.com/lukajaa/bay-clock-2" target="_blank">Source Code</a> / <a class="cursor-pointer" @click="toolsMenu = true">Tools</a> / <a class="cursor-pointer" @click="bugReport">Bug Report</a></p>
  </div>

  <div v-if="holiday != '' && holidayBool" class="aboslute-center">
    <img v-for="index in 50" :key="index" class="holiday-icon" :src="'images/icons/' + holidays[holiday]['icon']">
  </div>
</div>
</template>
...
import immersives from '../data/schedules/immersives.json';
import specialSchedules from '../data/schedules/special_schedules.json';
import activities from '../data/schedules/activity_schedule.json';
import breaks from '../data/schedules/breaks.json';
import holidays from '../data/schedules/holidays.json';
// guides
import colorGuide from '../data/guides/color_guide.json';
import colorPalette from '../data/guides/color_palette.json';
import usefulLinks from '../data/guides/useful_links.json';
import toolLinks from '../data/guides/tool_links.json';
...
const customSchedule = ref<ScheduleType>({A: 'A', B: 'B', C: 'C', D: 'D', E: 'E', F: 'F'})
...
const buttonColors = ref<StringString>({'Links': '', 'Menu': '', 'Schedule': '', 'Styles': '', 'Weekly': ''});
const colorBeingChosen = ref<string>('Links');
const toggles = ref({'Clock': true, 'Date': true, 'Time Left': true, 'Special Schedule Indicator': true});
...
const holiday = computed<string>(function() {
  ...
  for (const [name, info] of Object.entries(holidays)) {
    if (now > new Date(now.getFullYear() + '/' + info['start']) && now < new Date(now.getFullYear() + '/' + info['end'])) {
      return name
    }
  }
  return ''
...
onMounted(() => {
  ...
  if ($q.localStorage.getItem('holiday_bool')) {
    holidayBool.value = $q.localStorage.getItem('holiday_bool')
  }
})
